...
def run_weekly():
    if request.method == 'POST':
        ...
        print(monday)
        print(sunday)
        subprocess.run(f'python3 Predict.py {monday} {sunday}', shell=True, cwd = '..')
        # subprocess.run(f'python ContinuousTrain.py {monday} {sunday}', shell=True, cwd = '..')
        print(find_week(monday))
        if os.path.isfile(f'../History/Week {find_week(monday)}/Predictions.csv'):
            subprocess.run(f'python3 Hindsight.py {find_week(monday)-1}', shell=True, cwd = '..' )
